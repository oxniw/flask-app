...
def indexd():
    if request.method == "POST":
        ...
        with open(r"server\py\Include\database\userdata.json", "r") as f:
            userdata = json.load(f)
        with open(r"server\py\Include\database\usernameandpassword.json", "r") as f:
        ...
        if canuse and not username in usernameandpassword or len(username.split(" ")) > 1:
            ...
            with open(r"server\py\Include\database\userdata.json", "w") as f:
                json.dump(userdata,f,indent=4)
            with open(r"server\py\Include\database\usernameandpassword.json", "w") as f:
    ...
    if request.method == "POST":
        ...
        if friendname in userdatamyname["freinds"] and myname in userdatafriendname["freinds"] and friendname!=myname:
            ...
            while len(list(set(chatme) & set(chatmef))) < 1:
                ...
                with open(r"server\py\Include\database\chatdatabase.json","r") as f:
                    chatserver = json.load(f)
                chatserver.update({f"{encode}":{
                        "userinchat":[f"{myname}",f"{friendname}"],
                        "messageinchat":[]
                    }})
                with open(r"server\py\Include\database\chatdatabase.json","w") as f:
